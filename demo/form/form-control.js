...
import formControlData from './form-control-data.js';
import formControlStyle from './form-control-style.js';

const entries = Object.entries(formControlData);

class FormControl extends XElement {
  static #internalsMap = new WeakMap();

  // TODO: Consider adding this to the base class and return “false”.
  ...
  static createRenderRoot(host) {
    ...
    shadowRoot.adoptedStyleSheets = [formControlStyle];
    return shadowRoot;
  }
  ...
  static get properties() {
    return {
      ...
      emoji: {
        ...
        compute: value => {
          if (value) {
            const codePoint = formControlData[value];
            if (codePoint) {
  ...
  static validate(host) {
    const input = host.shadowRoot.getElementById('input');
    const internals = FormControl.#getInternals(host);
    const valueOk = !!formControlData[host.value];
