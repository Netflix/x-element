...
it('properties should not shadow XElement prototype interface', () => {
  ...
  try {
    ...
    const expected = 'Unexpected key "TestElement.properties.internal" shadows XElement.prototype interface (constructor, connectedCallback, attributeChangedCallback, adoptedCallback, disconnectedCallback, render, listen, unlisten, dispatchError, internal).';
...
it('properties should not shadow prototype chain interface', () => {
  ...
  console.warn = error => { // eslint-disable-line no-console
    message = error.message;
    if (error.message === 'Unexpected key "TestElement.properties.title" shadows inherited property name, behavior not guaranteed.') {
      passed = true;
    }
  };
  class TestElement extends XElement {
  ...
  customElements.define('test-element-warning', TestElement);
  console.warn = consoleWarn; // eslint-disable-line no-console
  assert(passed, message);
});

it('properties should only be from our known set', () => {
  let passed = false;
  let message = 'no error was thrown';
  try {
    class TestElement extends XElement {
