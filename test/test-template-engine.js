...
describe('html rendering', () => {
  ...
  it('renders escaped JS characters', () => {
    const container = document.createElement('div');
    render(container, html`
  ...
  it('renders “on*” attributes as event handlers', () => {
    ...
    render(container, html`<div onclick="this.textContent = '&hellip;hi\\u2026';"></div>`);
...
describe('html error formatting', () => {
  it('single line template', () => {
    const callback = () => html`<div id="target" not-ok=${'foo'}>Gotta double-quote those.</div>`;
    const expectedMessage = '[#128] Malformed attribute interpolation — attribute names must be alphanumeric (both uppercase and lowercase is allowed), must not start or end with hyphens, and cannot start with a number — and, attribute values must be enclosed in double-quotes.\nSee substring `not-ok=`.\nYour HTML was parsed through: `<div id="target" `.';
    assertThrows(callback, expectedMessage);
  });
  ...
  it('template with three lines', () => {
    ...
    const expectedMessage = '[#129] Malformed property interpolation — property names must be alphanumeric (both uppercase and lowercase is allowed), must not start or end with underscores, and cannot start with a number — and, property values must be enclosed in double-quotes.\nSee substring `.notOk=`.\nYour HTML was parsed through: `\n      \n      \n      <div id="target" `.';
    assertThrows(callback, expectedMessage);
  });
});
