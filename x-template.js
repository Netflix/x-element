...
class Unforgiving {
  ...
  static #throwTransitionError(strings, stringsIndex, string, stringIndex, value) {
    ...
    if (invalidValue) {
      message = [
        ...
        `See substring \`${notParsed}\`.`,
        `Your HTML was parsed through: \`${parsed}\`.`,
      ].join(' ');
    } else {
      message = [
        `Substring \`${notParsed}\` failed after ${Unforgiving.#debug(value)} state.`,
        `Your HTML was parsed through: \`${parsed}\`.`,
      ].join(' ');
    }
  ...
  static #validateRawString(rawString) {
    ...
    if (Unforgiving.#rawUnicodeOrHexadecimal.test(rawString)) {
      throw new Error([
        `Found a unicode or hexadecimal encoding (\\x or \\u) in raw string input.`,
        `Only valid HTML entities are supported in html.`,
        `This is the raw string leading up to the issue \`${rawString.slice(0, Unforgiving.#rawUnicodeOrHexadecimal.lastIndex)}\`.`,
      ].join(' '));
    }
  }
  ...
  static #addUnboundComment(string, stringIndex, element, childNodesIndex, nextStringIndex) {
    ...
    if (
      ...
      throw new Error([
        `Found malformed html comment.`,
        `Comments cannot start with a ">" character or "->" characters.`,
        `They cannot include a set of "--" characters.`,
        `They cannot end with a "-" character.`,
      ].join(' '));
    }
